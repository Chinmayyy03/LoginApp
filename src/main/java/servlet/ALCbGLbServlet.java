    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (securityTypeCodes != null && securityTypeCodes.length > 0) {
                String landBuildingSQL = "INSERT INTO APPLICATION.APPLICATIONSECURITYLANDNBULDIN (" +
                ...
                psLandBuilding = conn.prepareStatement(landBuildingSQL);
                int serialNum = 1;
                int validCount = 0;
                ...
                for (int i = 0; i < securityTypeCodes.length; i++) {
                    ...
                    // Skip if security type code is missing
                    if (secTypeCode == null || secTypeCode.isEmpty()) {
                        System.out.println("⚠️ Skip land & building " + (i+1) + " - missing security type");
                        continue;
                    }

                    System.out.println("✅ Land & Building " + serialNum + ": " + secTypeCode);

                    int colIdx = 1;

                    // 1. APPLICATION_NUMBER (NOT NULL, CHAR(14))
                    psLandBuilding.setString(colIdx++, applicationNumber);

                    // 2. SERIAL_NUMBER (NOT NULL, NUMBER(2,0))
                    psLandBuilding.setInt(colIdx++, serialNum);

                    // 3. SECURITYTYPE_CODE (NOT NULL, CHAR(50))
                    psLandBuilding.setString(colIdx++, secTypeCode);

                    // 4. SUBMISSIONDATE (DATE, nullable)
                    Date submiDate = lbSubmiDates != null && i < lbSubmiDates.length ?
                                    parseDate(lbSubmiDates[i]) : null;
                    if (submiDate != null) psLandBuilding.setDate(colIdx++, submiDate);
                    else psLandBuilding.setNull(colIdx++, Types.DATE);

                    // 5. VALUEAMOUNT (NUMBER(12,2), nullable)
                    Double amtValued = lbAmtValued != null && i < lbAmtValued.length ?
                                      parseDouble(lbAmtValued[i]) : null;
                    if (amtValued != null) psLandBuilding.setDouble(colIdx++, amtValued);
                    else psLandBuilding.setNull(colIdx++, Types.DECIMAL);

                    // 6. MARGINPERCENTAGE (NUMBER(5,2), nullable)
                    Double marginPct = lbMargins != null && i < lbMargins.length ?
                                      parseDouble(lbMargins[i]) : null;
                    if (marginPct != null) psLandBuilding.setDouble(colIdx++, marginPct);
                    else psLandBuilding.setNull(colIdx++, Types.DECIMAL);

                    // 7. AREA (NUMBER(10,3), nullable)
                    Double area = lbAreas != null && i < lbAreas.length ?
                                 parseDouble(lbAreas[i]) : null;
                    if (area != null) psLandBuilding.setDouble(colIdx++, area);
                    else psLandBuilding.setNull(colIdx++, Types.DECIMAL);

                    // 8. UNITOFAREA (CHAR(10), nullable)
                    String unitOfArea = lbUnitOfAreas != null && i < lbUnitOfAreas.length ?
                                       trimSafe(lbUnitOfAreas[i]) : null;
                    if (unitOfArea != null && !unitOfArea.isEmpty()) {
                        psLandBuilding.setString(colIdx++, unitOfArea);
                    } else {
                        psLandBuilding.setNull(colIdx++, Types.CHAR);
                    }

                    // 9. LOCATION (VARCHAR2(250), nullable)
                    String location = lbLocations != null && i < lbLocations.length ?
                                     trimSafe(lbLocations[i]) : null;
                    if (location != null && !location.isEmpty()) {
                        psLandBuilding.setString(colIdx++, location);
                    } else {
                        psLandBuilding.setNull(colIdx++, Types.VARCHAR);
                    }

                    // 10. SECURITYVALUE (NUMBER(12,2), nullable)
                    Double secValue = lbSecurityValues != null && i < lbSecurityValues.length ?
                                     parseDouble(lbSecurityValues[i]) : null;
                    if (secValue != null) psLandBuilding.setDouble(colIdx++, secValue);
                    else psLandBuilding.setNull(colIdx++, Types.DECIMAL);

                    // 11. REMARK (VARCHAR2(50), nullable)
                    String remark = lbRemarks != null && i < lbRemarks.length ?
                                   trimSafe(lbRemarks[i]) : null;
                    if (remark != null && !remark.isEmpty()) {
                        psLandBuilding.setString(colIdx++, remark);
                    } else {
                        psLandBuilding.setNull(colIdx++, Types.VARCHAR);
                    }

                    // 12. PARTICULAR (VARCHAR2(500), nullable)
                    ...
                    if (particular != null && !particular.isEmpty()) {
                        psLandBuilding.setString(colIdx++, particular);
                    } else {
                        psLandBuilding.setNull(colIdx++, Types.VARCHAR);
                    }

                    // 13. CREATED_DATE (TIMESTAMP(6), default SYSDATE)
                    psLandBuilding.setTimestamp(colIdx++, new Timestamp(System.currentTimeMillis()));

                    // 14. MODIFIED_DATE (TIMESTAMP(6), nullable)
                    psLandBuilding.setTimestamp(colIdx++, new Timestamp(System.currentTimeMillis()));

                    psLandBuilding.addBatch();
                    validCount++;
                    serialNum++;
                }

                if (validCount > 0) {
                    int[] landBuildingRows = psLandBuilding.executeBatch();
                    System.out.println("Land & Building records inserted: " + landBuildingRows.length);
                }
            }
