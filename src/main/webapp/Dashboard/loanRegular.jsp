...
<head>
<meta charset="UTF-8">
<title>Loan Regular - Branch <%= branchCode %></title>
...
function displayLoans(loans, page) {
    ...
    if (loans.length === 0) {
        tbody.innerHTML = "<tr><td colspan='7' class='no-data'>No regular loans found.</td></tr>";
...
function updatePaginationControls(totalRecords, page) {
    ...
    sessionStorage.setItem('loanRegularPage', page);
}
...
window.onload = function() {
    if (window.parent && window.parent.updateParentBreadcrumb) {
        window.parent.updateParentBreadcrumb('Dashboard > Loan Regular');
    }
    
    var savedPage = sessionStorage.getItem('loanRegularPage');
    if (savedPage) {
...
function viewLoan(accountCode) {
    if (window.parent && window.parent.updateParentBreadcrumb) {
        window.parent.updateParentBreadcrumb('Dashboard > Loan Regular > View Details');
    }
    window.location.href = '../View/viewAccount.jsp?accountCode=' + accountCode + '&returnPage=Dashboard/loanRegular.jsp';
}
</script>
</head>
<body>

<h2>Loan Regular for Branch: <%= branchCode %></h2>
...
<%
    ...
    try {
        ...
        // Fetch D_QUERY from DASHBOARD table for Loan Regular (SR_NUMBER = 9)
        String configQuery = "SELECT D_QUERY FROM GLOBALCONFIG.DASHBOARD WHERE SR_NUMBER = 9";
        ...
        if (rsConfig.next()) {
            ...
            if (dynamicQuery == null || dynamicQuery.trim().isEmpty()) {
                ...
                out.println("<p>No query defined in GLOBALCONFIG.DASHBOARD table (SR_NUMBER = 9).</p>");
            ...
            out.println("<p>Dashboard entry not found for SR_NUMBER = 9</p>");
            out.println("</div>");
            out.println("</td></tr>");
            return;
        }
        ...
        if (!hasData) {
            out.println("<tr><td colspan='7' class='no-data'>No regular loans found for this branch.</td></tr>");
        }
...
(function() {
    ...
    sessionStorage.setItem('loanRegularPage', '1');
})();
</script>
