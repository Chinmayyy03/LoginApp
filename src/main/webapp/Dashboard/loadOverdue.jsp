...
<head>
<meta charset="UTF-8">
<title>Loan Overdue - Branch <%= branchCode %></title>
...
function searchTable() {
    ...
    var table = document.getElementById("overdueTable");
    ...
    if (filter) {
        filteredLoans = allOverdueLoans.filter(function(loan) {
...
function displayLoans(loans, page) {
    ...
    if (loans.length === 0) {
        tbody.innerHTML = "<tr><td colspan='7' class='no-data'>No overdue loans found.</td></tr>";
...
function updatePaginationControls(totalRecords, page) {
    ...
    sessionStorage.setItem('loanOverduePage', page);
}

// Navigate to previous page
function previousPage() {
    var filter = document.getElementById("searchInput").value.toLowerCase().trim();
    var loans = allOverdueLoans;
    
    if (filter) {
        loans = allOverdueLoans.filter(function(loan) {
            return loan.accountCode.toLowerCase().indexOf(filter) > -1 ||
                   loan.name.toLowerCase().indexOf(filter) > -1;
        });
    }
...
window.onload = function() {
    ...
    if (savedPage) {
        ...
        displayLoans(allOverdueLoans, currentPage);
    }
};
...
<h2>Loan Overdue for Branch: <%= branchCode %></h2>
...
<%
    ...
    try {
        ...
        // Fetch D_QUERY from DASHBOARD table for Loan Overdue (SR_NUMBER = 10)
        String configQuery = "SELECT D_QUERY FROM GLOBALCONFIG.DASHBOARD WHERE SR_NUMBER = 10";
        ...
        if (rsConfig.next()) {
            ...
            if (dynamicQuery == null || dynamicQuery.trim().isEmpty()) {
                ...
                out.println("<p>No query defined in GLOBALCONFIG.DASHBOARD table (SR_NUMBER = 10).</p>");
            ...
            out.println("<p>Dashboard entry not found for SR_NUMBER = 10</p>");
            out.println("</div>");
            out.println("</td></tr>");
            return;
        }
        ...
        while (rsDynamic.next()) {
            ...
            out.println("<script>");
            out.println("allOverdueLoans.push({");
        ...
        if (!hasData) {
            out.println("<tr><td colspan='7' class='no-data'>No overdue loans found for this branch.</td></tr>");
        }
...
(function() {
    var totalPages = Math.ceil(allOverdueLoans.length / recordsPerPage);
    ...
    sessionStorage.setItem('loanOverduePage', '1');
})();
</script>
