...
<div class="dashboard-container">
    <div class="cards-wrapper">
        ...
        <div class="card" id="card-<%= srNumber %>"
             onclick="openInParentFrame('<%= pageLink %>', 'Master > <%= description %>')">
            <h3><%= description %></h3>
        ...
        <div class="error-message">
            <h3>No master cards configured</h3>
            <p>Please configure master cards in GLOBALCONFIG.MASTERS table</p>
        </div>
...
window.onload = function () {
    if (window.parent && window.parent.updateParentBreadcrumb) {
        window.parent.updateParentBreadcrumb('Master');
    }
...
async function loadSingleCard(card) {
    try {
        const response = await fetch('../getCardValueUnified.jsp?type=masters&id=' + card.srNumber);
...
function openInParentFrame(page, breadcrumbPath) {
    if (window.parent && window.parent.document) {
        ...
        if (iframe) {
            ...
            // If page doesn't contain '/', add '../' to go up from Master folder
