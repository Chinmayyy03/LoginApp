...
<%
    ...
    System.out.println("📌 ALCbGLb.jsp - Product Code received: " + productCode);
%>
...
<head>
  <meta charset="UTF-8">
<title>Loan Against Land & Building Application</title>
...
<form action="ALCbGLbServlet" method="post" onsubmit="return validateForm()">
  ...
  <select name="securityTypeCode[]" required>
    ...
    %>
      ...
      <div>
        ...
        <input type="number" step="0.01" name="lbAmtValued[]">
      </div>

      <div>
        <label>Margin %</label>
        <input type="number" step="0.01" name="lbMargin[]">
      </div>

      <div>
        <label>Area</label>
        <input type="number" step="0.01" name="lbArea[]">
      </div>
...
function addLandBuilding() {
  ...
  clone.querySelectorAll("input, select, textarea").forEach(el => {
      if (el.tagName === 'SELECT') {
          el.selectedIndex = 0;
      } else if (el.name === 'lbSrNo[]') {
          // Sr No will be updated by updateLBSerials()
          el.value = '';
...
function updateLBSerials() {
  ...
  blocks.forEach((block, index) => {
      ...
      // Update Sr No input field
      let srNoInput = block.querySelector('input[name="lbSrNo[]"]');
      if (srNoInput) {
