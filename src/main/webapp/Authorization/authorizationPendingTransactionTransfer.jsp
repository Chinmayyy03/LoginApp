...
<head>
<meta charset="UTF-8">
<title>Authorization Pending Transaction (Transfer) - Branch <%= branchCode %></title>
    ...
    if (filter) {
        filtered = allTransactions.filter(function(t) {
            return t.scrollNumber.toLowerCase().indexOf(filter) > -1 ||
                   t.subscrollNumber.toLowerCase().indexOf(filter) > -1 ||
...
function updatePaginationControls(totalRecords, page) {
    ...
    sessionStorage.setItem('authPendingTxnTransferPage', page);
}
...
window.onload = function() {
    ...
    var savedPage = sessionStorage.getItem('authPendingTxnTransferPage');
    if (savedPage) {
...
<h2>Authorization Pending Transaction (Transfer) — Branch: <%= branchCode %></h2>
...
try (Connection conn = DBConnection.getConnection()) {
    ...
    PreparedStatement ps = conn.prepareStatement(
        ...
        "ORDER BY d.SCROLL_NUMBER, d.SUBSCROLL_NUMBER"
    );
    ...
    while (rs.next()) {
        ...
        String subscrollNumber = rs.getString("SUBSCROLL_NUMBER") != null ? rs.getString("SUBSCROLL_NUMBER") : "";
        ...
        out.println("  subscrollNumber: '" + subscrollNumber + "',");
        ...
        if (displayCount < recordsPerPage) {
            ...
            out.println("<td>" + subscrollNumber + "</td>");
    ...
    if (!hasData) {
        out.println("<tr><td colspan='10' class='no-data'>No pending transfer transactions found.</td></tr>");
    }

} catch (Exception e) {
    out.println("<tr><td colspan='10' class='no-data'>Error: " + e.getMessage() + "</td></tr>");
}
%>
</tbody>
</table>
...
(function() {
    ...
    sessionStorage.setItem('authPendingTxnTransferPage', '1');
})();
</script>
