...
// Fetch users - CURRENTLOGIN_STATUS = 'L' means live/logged-in
...
<head>
    ...
    .filter-card form {
        display: grid;
        grid-template-columns: 1fr 1fr auto 110px;
    ...
    .radio-row { display: flex; align-items: center; gap: 20px; height: 34px; }
    .radio-row label { display: flex; align-items: center; gap: 5px; font-size: 13px; color: #444; cursor: pointer; }
    .radio-row input[type="radio"] { accent-color: #2b0d73; width: 14px; height: 14px; cursor: pointer; }

    .filter-btn {
        height: 34px; width: 100%;
        background-color: #2b0d73; color: #FFFFFF;
        border: none; border-radius: 3px;
        font-size: 13px; font-weight: 600; cursor: pointer;
    }
    .filter-btn:hover { background: #1E2870; }
...
<div class="container">
    ...
    <div class="filter-card">
        <form method="get" action="">
            ...
            <div class="status-group">
                ...
                <div class="radio-row">
                    <label>
                        <input type="radio" name="status" value="all" <%="all".equals(selectedStatus) ? "checked" : ""%>>
                        All
                    </label>
                    <label>
                        <input type="radio" name="status" value="live" <%="live".equals(selectedStatus) ? "checked" : ""%>>
                        Live
                    </label>
                </div>
            </div>
    ...
    <div class="table-card">
        <table id="usersTable">
            ...
            <tbody id="tableBody">
                <%-- Only render data rows here, no-records message handled by JS --%>
                <%
                    if (!userList.isEmpty()) {
                        int sr = 1;
...
<script>
    ...
    function renderPage() {
        ...
        if (totalFromServer === 0) {
            ...
            // Data exists but search filtered everything out
            const tbody = document.getElementById('tableBody');
            const tr = document.createElement('tr');
            tr.id = 'dynamicNoRecord';
